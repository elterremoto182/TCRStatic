...
interface CommonCausesListProps {
  content: CommonCausesListContent;
}

export function CommonCausesList({ content }: CommonCausesListProps) {
  if (!content.causes || content.causes.length === 0) {
    return null;
  }

  // Split into three columns for larger screens
  const colSize = Math.ceil(content.causes.length / 3);
  const col1 = content.causes.slice(0, colSize);
